...
<form name="frmMain" method="post" id="frmMain" action="<?php echo $this->get_controller_url();?>dsp_single_user_to_grand"><?php
    ...
    <div class="button-area">
        <input type="button" name="btn_update_user_permit" class="ButtonAccept" value="<?php echo _LANG_UPDATE_BUTTON; ?>" onclick="btn_update_user_permit_onclick(); "/>
        <?php $v_back_action = ($v_pop_win === '') ? 'btn_back_onclick();' : 'try{window.parent.hidePopWin();}catch(e){window.close();};';?>
        <input type="button" name="cancel" class="ButtonCancel" value="<?php echo _LANG_CANCEL_BUTTON; ?>" onclick="<?php echo $v_back_action;?>"/>
    </div>
</form>
