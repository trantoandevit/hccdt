...
//        $v_keywords     = str_replace('-', ' and ', $v_keywords);
        ...
        if ($v_begin_date != '')
        {
            ...
            $v_filter_condition .= " And (date_format(C_BEGIN_DATE,'%Y-%m-%d') >= '$v_begin_date')";
        }
        ...
        if ($v_end_date != '')
        {
            ...
            $v_filter_condition .= " And (date_format(C_END_DATE,'%Y-%m-%d') <= '$v_end_date')";
        }
        ...
        if ($v_keywords)
        {
            ...
            {
                $a_other_clause .= " And Contains((C_TITLE, C_TAGS, C_SUMMARY, C_CONTENT, C_KEYWORDS, C_SUB_TITLE), '$v_keywords', language 1066)";
            }
        ...
        if ($v_title != '')
        {
            if(DATABASE_TYPE == 'MSSQL')
            {
                $a_other_clause .= " And C_TITLE Like '%$v_title%'";
            }
            elseif(DATABASE_TYPE == "MYSQL")
            {
        ...
        if(DATABASE_TYPE == 'MSSQL')
        {
            $sql = "
                ...
                Where TEMP.RN Between ? And ?
                ";

            $sql = "
                Select TEMP.*
                    ,A.C_TITLE, A.C_MESSAGE
                    ,U.C_NAME As C_INIT_USER_NAME, A.C_SLUG, A.C_BEGIN_DATE as C_REAL_DATE,
                    C_XML_VERSION.value('(//version[status=3][last()]/user_name)[1]','Nvarchar(255)') as C_SENSORER
                    ,convert(varchar, A.C_BEGIN_DATE, 103) as C_BEGIN_DATE, A.C_STATUS, C.PK_CATEGORY
                    ...
                    ,(
                        Select Count(*) From t_ps_article_COMMENT AC 
                        Where C_STATUS = 0 And AC.FK_ARTICLE = A.PK_ARTICLE
                    ) as C_COMMENT_COUNT
                From ($sql) TEMP 
                inner Join t_ps_article A
                On TEMP.PK_ARTICLE = A.PK_ARTICLE
                inner Join t_ps_category C
                ON TEMP.PK_CATEGORY = C.PK_CATEGORY
                left Join t_cores_user U
                On TEMP.FK_INIT_USER = U.PK_USER
                Order By TEMP.RN";
        ...
        else if(DATABASE_TYPE == 'MYSQL')
        {
            ...
            $v_query_count_all_article = "Select 
                                    ...
                                    Where C_DEFAULT_WEBSITE=$v_website_id $v_filter_condition ";
                ...
                $v_query_count_all_article = "SELECT
                                              ...
                                              FROM t_ps_category_article CA USE INDEX(FK_ARTICLE)
                                                LEFT JOIN t_ps_article A 
                                                  ON CA.FK_ARTICLE = A.PK_ARTICLE
                                              WHERE A.C_DEFAULT_WEBSITE = $v_website_id
    ...
    function qry_all_website()
    {
        $sql = 'Select PK_WEBSITE, C_NAME From t_ps_website';
        return $this->db->getAll($sql);
    }
    ...
    function title_exists($id, $title)
    {
        ...
        $sql    = "Select count(*) From t_web_article Where C_TITLE = ? And PK_ARTICLE <> ? and C_DEFAULT_WEBSITE =  '$v_website_id'";
    ...
    function qry_single_article($id, $a_other_clause = '', $c_other_clause = '')
    {
        ...
        $sql             = "
            Select 
                A.PK_ARTICLE, A.C_TITLE, A.C_MESSAGE, A.FK_INIT_USER
                , A.C_HAS_VIDEO, A.C_HAS_PHOTO,A.C_IS_COPY,A.C_IS_IMG_NEWS,A.FK_EMPLOYEE,
                DATE_FORMAT(A.C_BEGIN_DATE,'%Y-%m-%d %h:%i:%s') AS C_BEGIN_DATE,
                DATE_FORMAT(A.C_END_DATE,'%Y-%m-%d %h:%i:%s') AS C_END_DATE,
                A.C_STATUS, A.C_SUB_TITLE, A.C_SUMMARY, A.C_CONTENT, A.C_SLUG, 
                A.C_KEYWORDS, A.C_TAGS, A.C_VIEWS, A.C_XML_VERSION, A.C_PEN_NAME,
                U.C_NAME as C_INIT_USER_NAME, A.C_FILE_NAME
    ...
    function qry_category_article($id,$no_website = '0')
    {
        $sql = '
        ...
        if($no_website == '0')
        {
            $sql .= ' And C.FK_WEBSITE = ' . Session::get('session_website_id');
        }
...
//        $v_content    = htmlspecialchars($v_content);
        ...
        $v_keywords   = get_request_var('txt_keywords');
        ...
        $v_end_date   = '2100/1/1';
        $v_has_video  = (int) get_post_var('chk_has_video');
        $v_has_photo    = (int) get_post_var('chk_has_photo');
        ...
        if ($v_id == 0)
        {
            ...
            if(DATABASE_TYPE == 'MSSQL')
            {
                $v_id = intval($this->db->getOne("Select IDENT_CURRENT('t_ps_article')"));
            }
            else if(DATABASE_TYPE == 'MYSQL')
            {
        ...
        else //update
        {
            ...
            else
            {
                ...
                if (get_system_config_value(CFGKEY_CACHE) == 'true')
                {
                    $website_id = session::get('session_website_id');
                    $cache = new GP_Cache();
                    $v_success = $cache->create_latest_article_cache($website_id);
                }
            }
        } //het update
        ...
        //tin bai nao thuoc chuyen muc video phai co bieu tuong video
        //$this->update_article_invideo();

        $this->db->debug = DEBUG_MODE;
        return $arr_msg;
    }

    function update_article_invideo()
    {
        if(DATABASE_TYPE == 'MSSQL')
        {
            $sql = "
                Update t_ps_article Set C_HAS_VIDEO = 1
                From t_ps_article A
                Inner Join t_ps_category_article CA
                On CA.FK_ARTICLE = A.PK_ARTICLE
                Inner Join t_ps_category C
                On C.PK_CATEGORY = CA.FK_CATEGORY
                Where C.C_IS_VIDEO > 0
            ";
            $this->db->Execute($sql);
        }
        else if(DATABASE_TYPE == 'MYSQL')
...
//  function update_article()
    ...
    function update_category_article($article_id, $arr_category)
    {
        ...
        if (!empty($arr_delete))
        {
            ...
            $sql = "Delete From T_PS_EVENT_ARTICLE 
                Where FK_ARTICLE = $article_id And FK_CATEGORY In($arr_delete)";
            $this->db->Execute($sql);
...
// function update_category_article()
    ...
    function update_edited_article()
    {
        ...
        $v_begin_date = str_replace('/', '-', $v_begin_date);
        $v_end_date   = str_replace('/', '-', $v_end_date);
...
// end function update_edited_article
    ...
    function update_article_sticky($article_id, $arr_sticky_category)
    {
        ...
        //insert
        $n = count($arr_add);
        $arr_add = array_values($arr_add);
        for ($i = 0; $i < $n; $i++)
        {
            $category_id = (int) $arr_add[$i];
            $sql         = 'Insert Into t_ps_sticky(FK_CATEGORY, FK_ARTICLE, C_DEFAULT, FK_WEBSITE)
                VALUES(?,?,?,?)';
            $params      = array($category_id, $article_id, 0, $website_id);
            $this->db->execute($sql, $params);
            $this->build_order('t_web_sticky',
                    'PK_STICKY',
                    'C_ORDER',
        ...
        if (!empty($arr_remove))
        {
            ...
            $params     = array($website_id, $article_id);
            $this->db->execute($sql, $params);
        }
    }
    ...
    function qry_sticky_category($article_id)
    {
        $sql    = 'Select FK_CATEGORY From t_ps_sticky
            ...
            AND FK_WEBSITE = ?
            AND C_DEFAULT = 0';
        $params = array($article_id, Session::get('session_website_id'));
    ...
    /**
     * Lay danh sach chuyen muc da duoc phan cong tac nghiep
     * @return array 
     */
    ...
    function qry_all_grant_category()
    {
        ...
        $arr_super_group = array('ADMINISTRATORS', 'TONG_BIEN_TAP');
    ...
    function delete_article($a_other_clause = '', $c_other_clause = '')
    {
        ...
        $arr_fk = array(
            ...
            , 't_ps_event_article'      => 'fk_article'
            , 't_ps_spotlight'          => 'fk_article'
            , 't_ps_sticky'             => 'fk_article'
        );
        ...
        $sql = "Delete From t_ps_category_article Where FK_ARTICLE IN($arr_delete)";
        $this->db->Execute($sql);
    }
    ...
    {
        $key = OPT_TAGS;
        $sql = "Select C_OPTION_VALUE From t_ps_option Where C_OPTION_KEY ='$key'";
        return $this->db->getOne($sql);
    }
    ...
    function count_all_article($other_clause = '')
    {
        ...
        $sql = "Select 
               ...
               Where A.C_DEFAULT_WEBSITE = $website $other_clause";
        return $this->db->getOne($sql);
    }
    ...
    private function save_tags($str_tags)
    {
        ...
        {
            $sql     = "
                    If Exists(Select PK_OPTION From t_ps_option Where C_OPTION_KEY = '$key')
                        Update t_ps_option Set C_OPTION_VALUE = '$db_data' Where C_OPTION_KEY = '$key'
                    Else
                        Insert Into t_ps_option(C_OPTION_KEY, C_OPTION_VALUE) Values('$key', '$db_data')
                ";
                $this->db->Execute($sql);
        }
        else if(DATABASE_TYPE == 'MYSQL')
        {
            $sql = "Select COUNT(PK_OPTION) From t_ps_option Where C_OPTION_KEY = '$key'";
            $check = $this->db->getOne($sql);
            
            if($check > 0)
            {
                $sql = "Update t_ps_option Set C_OPTION_VALUE = '$db_data' Where C_OPTION_KEY = '$key'";
                $this->db->Execute($sql);
            }
            else
            {
                $sql = "Insert Into t_ps_option(C_OPTION_KEY, C_OPTION_VALUE) Values('$key', '$db_data')";
                $this->db->Execute($sql);
            }
        }
    ...
    //lay tat ca tin bai duoc phep kiem duyet
    public function qry_all_approve_article( $a_other_clause = '', $c_other_clause = '', $ca_other_clause = '')
    {
        $v_title = trim(get_post_var('hdn_search',''));
        ...
        else if(DATABASE_TYPE == 'MYSQL')
        {
            ...
            $sql = "SELECT
                        ...
                        lim_A.C_TITLE,
                        lim_A.C_FILE_NAME,
                        lim_A.C_BEGIN_DATE AS C_REAL_DATE,
                        DATE_FORMAT(lim_A.C_BEGIN_DATE,'%d-%m-%Y') AS C_BEGIN_DATE,
                        W.C_NAME,
                       (SELECT C_NAME FROM t_ps_category WHERE PK_CATEGORY = lim_A.C_DEFAULT_CATEGORY) AS C_DEFAULT_CATEGORY_NAME,
                       lim_A.C_PEN_NAME,
                       E.C_NAME AS C_AUTHOR 
                      FROM ($sql) lim_A
                        LEFT JOIN t_ps_website W 
                          ON lim_A.C_DEFAULT_WEBSITE  = W.PK_WEBSITE
                        LEFT JOIN t_ps_employee E
                            ON lim_A.FK_EMPLOYEE = E.PK_EMPLOYEE ";
            $param = array($v_start,$v_limit) ;
        }
        return $this->db->getAll($sql, $param);
    }
    ...
    public function qry_all_employee()
    {
        $sql = "SELECT PK_EMPLOYEE,C_NAME FROM t_ps_employee";
        return $this->db->getAll($sql);
    }
    
    //lay du lieu tin bai khong can website_id 
    public function qry_single_approve_article($id, $a_other_clause = '', $c_other_clause = '')
    {
        $user_id         = Session::get('user_id');
        $sql_sub_article = "Select PK_ARTICLE From t_ps_article Where PK_ARTICLE = $id $a_other_clause";
        $sql_category    = "Select PK_CATEGORY, C_SLUG, C_NAME From t_ps_category Where 1=1 $c_other_clause";
        $sql             = "
            Select 
                A.PK_ARTICLE, A.C_TITLE, A.C_MESSAGE, A.FK_INIT_USER
                , A.C_HAS_VIDEO, A.C_HAS_PHOTO,A.C_IS_COPY,A.FK_EMPLOYEE,
                DATE_FORMAT(A.C_BEGIN_DATE,'%Y-%m-%d %h:%i:%s') AS C_BEGIN_DATE,
                DATE_FORMAT(A.C_END_DATE,'%Y-%m-%d %h:%i:%s') AS C_END_DATE,
                A.C_STATUS, A.C_SUB_TITLE, A.C_SUMMARY, A.C_CONTENT, A.C_SLUG, 
                A.C_KEYWORDS, A.C_TAGS, A.C_VIEWS, A.C_XML_VERSION, A.C_PEN_NAME
