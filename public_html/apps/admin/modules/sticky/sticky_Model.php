...
class sticky_Model extends Model
{
    ...
    function insert_sticky()
    {
        ...
        $v_cat_id    = $arr_article[0]['article_category_id']; 
        ...
        if($v_default == 2) // sap xep lai thu tu tin noi bat trong ngay
        {
            $this->build_order('t_ps_sticky', 'PK_STICKY', 'C_ORDER', " AND FK_WEBSITE=$v_website And C_TYPE='2'");
        }
        else if($v_default == 1) // sap xep lai thu tu tin noi bat trang chu
        {
            $this->build_order('t_ps_sticky', 'PK_STICKY', 'C_ORDER', " AND FK_WEBSITE=$v_website And C_DEFAULT='$v_default'");
        }
        else // sap xep lai tin noi bat chuyen muc
        {
            $this->build_order('t_ps_sticky', 'PK_STICKY', 'C_ORDER', " AND FK_WEBSITE=$v_website And C_DEFAULT='$v_default' and FK_CATEGORY = '$v_cat_id'");
        }
