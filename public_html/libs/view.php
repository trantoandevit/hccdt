...
class View
{
    ...
    function __construct($app, $module)
    {
        ...
        $this->template->dsp_side_bar = true;
    }

    private function _render_error($code, $filename)
    {
        switch ($code)
        {
            case 1:
                if (DEBUG_MODE == 1)
                {
                    die("Lỗi view render, không tìm thấy file <b>$filename</b>");
                }
    ...
    public function get_controller_url($module = NULL, $app = NULL,$v_args = NULL)
    {
        if(empty($v_args))
        {
            $v_args  = trim($v_args);
        ...
        if (file_exists('.htaccess'))
        {
            ...
            return SITE_ROOT . $app . '/' . $module . '/'.$v_args;
        }
        return SITE_ROOT . 'index.php?url=' . $app . '/' . $module . '/'.$v_args;
    }

    public function render($name, $VIEW_DATA = array(), $theme = '')
    {
        if ($theme == '')
        {
        ...
        {
            $v_view_file = SERVER_ROOT . 'apps' . DS . $this->app_name . DS . 'themes' . DS . $theme . DS . $name . '.php';
        }
    ...
    public function transform($full_path,$xml_string = '')
    {
        $this->xslt = new Xslt();
        if(trim($xml_string) != '')
        {
            $this->xslt->setXmlString($xml_string);
        }
        else 
        {
        ...
        $html       = '';
        $this->xslt->setXslString(file_get_contents(SERVER_ROOT . 'libs/Transform.xslt'));
    ...
    {
        $v_site_root = SITE_ROOT;
        if (!( preg_match( '/^http:\/\//', $v_site_root) == 1 ))
        {
            $v_site_root = 'http://' . $_SERVER['HTTP_HOST'] . $v_site_root;
        }
        ...
        $this->xslt->setXslString(file_get_contents(SERVER_ROOT . 'libs/Transform.xslt'));
        $this->xslt->setParameter(array('p_site_root'    => $v_site_root, 'p_current_date' => date('d-m-Y')));
        if ($this->xslt->transform())
        {
            $html = $this->xslt->getOutput();
            $this->xslt->destroy();
        ...
        for ($i = 0; $i < sizeof($data); $i++)
        {
            ...
            foreach ($cols as $col)
            {
                ...
                switch (strval($col->attributes()->type))
                {
                    ...
                    case 'checkbox':
                        $v_disabled = (isset($data[$i]['COUNT_DEPEND']) && $data[$i]['COUNT_DEPEND'] > 0) ? ' disabled' : '';
                        $html .= '<td class="center"><input type="checkbox" name="chk"
                            value="' . $data[$i][$primarykey] . '"' . $v_disabled . '
...
//end func add_empty_rows
    ...
    public static function generate_select_option($arrData, $selected, $public_xml_file_name = '')
    {
        $html = '';
    ...
    public static function paging($page, $rows_per_page, $total_record)
    {
        ...
        $html .= VIEW::generate_select_option($arr_page, $page);
        $html .= '</select>';

        $html .= __('display') . '<select name="sel_rows_per_page" onchange="this.form.sel_goto_page.value=1;this.form.submit();">';
        $html .= VIEW::generate_select_option(null, $rows_per_page, 'xml_rows_per_page.xml');
...
//        }
        ...
        $html .= '. ' . __('go to') . '<input type="textbox" name="sel_goto_page" value="'.$page.'" size="5" />';
...
//        $html .= '</select>';
        ...
        $html .= '<input type="button" value="Lọc" onclick="this.form.submit();">';
        $html .= '</div>';
